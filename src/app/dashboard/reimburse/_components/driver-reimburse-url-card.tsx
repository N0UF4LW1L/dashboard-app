...
export default function DriverReimburseUrlCard() {
    ...
    const [url, setUrl] = useState('/driver-reimburse');

    useEffect(() => {
        if (typeof window !== 'undefined') {
            setUrl(`${window.location.origin}/driver-reimburse`);
        }
    }, []);

    const handleCopy = () => {
        const fullUrl = typeof window !== 'undefined'
            ? `${window.location.origin}/driver-reimburse`
            : '/driver-reimburse';
