  ...
  const fetchTransactionData = async () => {
    ...
    try {
      const [transactionRes, categoriesRes] = await Promise.all([
        apiClient.get(`/realization/financial-transactions/${transactionId}`),
        apiClient.get(`/realization/transaction-categories`, { params: { page: 1, limit: 1000 } })
      ]);
      
      const transactionData = transactionRes.data?.data || transactionRes.data;
      const categoriesData = categoriesRes.data?.data || categoriesRes.data?.items || [];
