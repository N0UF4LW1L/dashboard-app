    ...
    <>
      <ScrollArea className="h-[calc(80vh-200px)]">
        <div className="overflow-x-auto min-w-full">
          <table className="w-full border-collapse border border-gray-300 min-w-[600px] sm:min-w-[800px]">
            <thead>
                ...
                {table.getHeaderGroups().map((headerGroup) => (
                  headerGroup.headers.map((header) => {
                    ...
                    return (
                      <th
                        key={header.id}
                        className={`py-2 sm:py-3 px-2 sm:px-4 font-semibold text-gray-800 text-left text-xs sm:text-sm whitespace-nowrap ${
                        ...
                        return (
                          <td
                            ...
                            className={`py-2 sm:py-3 px-2 sm:px-4 text-gray-900 text-xs sm:text-sm ${
                              isLastColumn ? '' : 'border-r border-gray-300'
                            } ${columnId === 'actions' ? 'text-center' : ''} ${columnId === 'debit' || columnId === 'kredit' ? 'whitespace-nowrap text-right' : ''}`}
